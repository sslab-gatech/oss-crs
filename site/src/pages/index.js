...
const SETUP_OAUTH = `claude setup-token

# Export the token, or add it to a .env file in your working directory
export CLAUDE_CODE_OAUTH_TOKEN=<your-oauth-token>`;

const RUN_CLAUDE_CODE_BUG_FINDING = `uv run oss-crs prepare \\
...
const RUN_CLAUDE_CODE_PATCHING = `uv run oss-crs prepare \\
  ...
  return (
    <section id="quick-start" className="section section--alt">
      <div className="container">
        ...
        <h3>5. Run Claude Code CRSs with OAuth</h3>
        <p>
          We also provide configurations for running our Claude Code bug-finding and patching CRSs
          using an OAuth token, which can be generated by a user with an Anthropic subscription.
        </p>
        <CodeBlock language="bash">{SETUP_OAUTH}</CodeBlock>
        <p>Bug-finding with Claude Code:</p>
        <CodeBlock language="bash">{RUN_CLAUDE_CODE_BUG_FINDING}</CodeBlock>
        <p>Bug-fixing (patching) with Claude Code against a provided PoV:</p>
        <CodeBlock language="shell-session">{RUN_CLAUDE_CODE_PATCHING}</CodeBlock>
